...
export function replyCodeToMessage(reply: AutoUpdate.Reply) {
	switch (reply.code) {
		...
		case AutoUpdate.ReplyCode.SUCCESS:
			return `A website update has successfully been initiated. You can view the changes by <a href="https://github.com/Vic-Park/vpciclubs.com/commit/${reply.data.commitSha}">clicking here.</a>`;
